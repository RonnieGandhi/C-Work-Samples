...
static void show_usage(const char *cmd)
{
  //fprintf(stderr, "chrt (%s)\n", PACKAGE_STRING);
	...
	fprintf(stderr, "  -x, --background                         "
			"set policy to SCHED_BACKGROUND\n");
...
static void show_min_max(void)
{
	...
	max = sched_get_priority_max(SCHED_BACKGROUND);
	min = sched_get_priority_min(SCHED_BACKGROUND);
	if (max >= 0 && min >= 0)
		printf("SCHED_BACKGROUND min/max priority\t: %d/%d\n", min, max);
	else
		printf("SCHED_BACKGROUND not supported?\n");
